...
import jakarta.validation.constraints.Size;
 ...
 public record SignInCommand(
         ...
         @Parameter(description = "계정명. 이 필드는 필수입니다.", required = true)
         @Schema(description = "계정명. 이 필드는 필수입니다.", example = "kalgooksoo")
         @Size(min = 3, max = 20, message = "계정명은 3자 이상 20자 이하이어야 합니다")
         @NotBlank(message = "계정명은 필수입니다")
         @NotNull(message = "계정명은 NULL이 될 수 없습니다")
         String username,

         @Parameter(description = "패스워드. 이 필드는 필수입니다.", required = true)
         @Schema(description = "패스워드. 이 필드는 필수입니다.", example = "password", format = "password")
